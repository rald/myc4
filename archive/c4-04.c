...
enum { LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,
       ...
       OPEN,READ,WRIT,CLOS,PRTF,SCNF,SPRF,SSCF,MALC,FREE,MSET,MCMP,MCPY,PCHR,GCHR,EXIT };
...
void next()
{
  ...
  while ((tk = *p)) {
    ...
    if (tk == '\n') {
      if (src) {
        printf("%ld: %ld %s", line, p - lp, lp);
        lp = p;
        while (le < e) {
          printf("%8.4s", &"LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,"
                           "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
                           "OPEN,READ,WRIT,CLOS,PRTF,SCNF,SPRF,SSCF,MALC,FREE,MSET,MCMP,EXIT,"[*++le * 5]);
          if (*le <= ADJ) printf(" %ld\n", *++le); else printf("\n");
        }
      }
      ++line;
...
int main(int argc, char **argv)
{
  ...
  LONG *pc, *sp, *bp, a=0L, cycle; // vm registers
  LONG i, *t; // temps
  ...
  p = "char else enum if int return sizeof while "
      "open read write close printf scanf sprintf sscanf malloc free memset memcmp exit void main";
  ...
  *--sp = (LONG)t;
  ...
  while (1) {
    ...
    if (debug) {
      printf("%ld> %.4s", cycle,
        &"LEA ,IMM ,JMP ,JSR ,BZ  ,BNZ ,ENT ,ADJ ,LEV ,LI  ,LC  ,SI  ,SC  ,PSH ,"
         "OR  ,XOR ,AND ,EQ  ,NE  ,LT  ,GT  ,LE  ,GE  ,SHL ,SHR ,ADD ,SUB ,MUL ,DIV ,MOD ,"
         "OPEN,READ,WRIT,CLOS,PRTF,SCNF,SPRF,SSCF,MALC,FREE,MSET,MCMP,MCPY,PCHR,GCHR,EXIT,"[i * 5]);
      if (i <= ADJ) printf(" %ld\n", *pc); else printf("\n");
    }
    ...
    else if (i == SCNF) { t = sp + pc[1]; a = scanf((char *)t[-1], t[-2], t[-3], t[-4], t[-5], t[-6]); }
    else if (i == SPRF) { t = sp + pc[1]; a = sprintf((char *)t[-1], (char *)t[-2], t[-3], t[-4], t[-5], t[-6]); }
    else if (i == SSCF) { t = sp + pc[1]; a = sscanf((char *)t[-1], (char *)t[-2], t[-3], t[-4], t[-5], t[-6]); }
    else if (i == MALC) a = (LONG)malloc(*sp);
    else if (i == FREE) free((void *)*sp);
    else if (i == MSET) a = (LONG)memset((char *)sp[2], sp[1], *sp);
    else if (i == MCMP) a = memcmp((char *)sp[2], (char *)sp[1], *sp);
    else if (i == MCPY) a = (LONG)memcpy((char *)sp[2], (char *)sp[1], *sp);
